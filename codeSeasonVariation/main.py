...
from networks import VGGWithEurosat_32_32,ResNetFcFor32_32
...
parser.add_argument('--dataset', type=str, default='office31', choices=['office31', 'office-home', 'pacs',
                                                                        'domain-net','lcz42CityVariation','sen12SeasonVariation'], help='dataset used')
...
#    if args.dataset=='lcz42CityVariation':
#        base_network = ResNetFcFor32_32('ResNet18', use_bottleneck=True, bottleneck_dim=256, new_cls=True, class_num=8)
    ...
    targetDomainCounter = 0
    for _ in range(len(config['data']['target']['name'])):
    
        ##Part added by Sudipan
        targetDomainCounter = targetDomainCounter+1
        print(targetDomainCounter)
        if targetDomainCounter==2: ##don't add for 1st target domain, starting adding from the 2nd one
            config['threshold']=config['threshold']+0.05
        elif targetDomainCounter==3:
            config['threshold']=config['threshold']+0.05
        elif targetDomainCounter==4:
            config['threshold']=config['threshold']+0.05

        #print(config['threshold'])
        
        ##Part added by Sudipan ends
