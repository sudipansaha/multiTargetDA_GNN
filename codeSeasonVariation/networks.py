...
class ResNetFc(nn.Module):
    def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
        ...
        self.instanceNormForInput = nn.InstanceNorm2d(3)
        ...
        self.feature_layers = nn.Sequential(self.instanceNormForInput,self.conv1, self.bn1, self.relu, self.maxpool, self.layer1,
...
class ResNetFcFor32_32(nn.Module):
    def __init__(self, resnet_name, use_bottleneck=True, bottleneck_dim=256, new_cls=False, class_num=1000):
        super(ResNetFcFor32_32, self).__init__()
        model_resnet = resnet_dict[resnet_name](pretrained=True)
        
        
        self.instanceNormForInput = nn.InstanceNorm2d(3)  ##3 - number of input channel
        self.conv1 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1,bias=False)  ##make conv1 3*3 instead of 7*7
        self.bn1 = model_resnet.bn1
        self.relu = model_resnet.relu
        self.maxpool = model_resnet.maxpool
        self.layer1 = model_resnet.layer1
        ...
        self.avgpool = model_resnet.avgpool
        self.feature_layers = nn.Sequential(self.instanceNormForInput,self.conv1, self.bn1, self.relu, self.maxpool, self.layer1,
                                            self.layer2, self.layer3, self.avgpool)
        self.use_bottleneck = use_bottleneck
        self.new_cls = new_cls
        if new_cls:
            if self.use_bottleneck:
...
####Functions and class related to VGG
...
class VGGWithEurosat_32_32(nn.Module):
    def __init__(self, n_inputs = 3, numCls = 10,pretrained=False):  # num. of classes
        super().__init__()
        ...
        self.feature_layers = nn.Sequential(
            nn.Conv2d(n_inputs, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1)),*vgg.features[1:17]   ##taking upto 3rd pooling layer
        )
        self.bottleneck = nn.Linear(4*4*256, 256, bias=True) # 8*8*512: output size from encoder (origin img pixel 32*32-> after 3 pooling size is 4)

        self.fc = nn.Linear(256,numCls,bias=True)
        
        self.__in_features = 256 ##bottleneck_dim
